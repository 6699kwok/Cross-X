...
// ─── Coze ─────────────────────────────────────────────────────────────────────
let COZE_API_KEY = "";
let COZE_BOT_API_KEY = ""; // separate PAT for bot (may be in a different space)
...
// (buildSyntheticEnrichment, buildAIEnrichment, callCozeWorkflow extracted to src/services/coze.js)
...
// (queryAmapPoi, queryAmapHotels, enrichPlanWithAmapData extracted to src/services/amap.js)

// (queryJuheFlight, queryJuheFlightInvoice extracted to src/services/juhe.js)
...
// [P4] toNumberOrNull, haversineKm, inferCityFromCoordinates,
// inferCityNameFromCoordinates, wgs84ToGcj02, reverseGeocodeWithAmap
// → extracted to src/services/geo.js
...
// [P4] offsetCoords → extracted to src/services/geo.js
...
async function callCasualChat({ message, language, city, history = [] }) {
  const lang = normalizeLang(language);
...
- Do NOT generate travel plans or itineraries here — that's handled separately`;
  ...
  // Build multi-turn messages: system + last 6 history turns + current user message
...
async function executeTask(task) {
  ...
  const bookOutput     = (result.outputs || []).find((o) => o.toolType === "book.lock" || o.toolType === "transport.lock");
  const payData        = (payOutput && payOutput.data) || {};
  const bookData       = (bookOutput && bookOutput.data) || {};
  const merchantRef    = bookData.lockId || bookData.ticketRef || payData.paymentRef || null;

  const proofWithQr = { ...(result.proof || {}), _payData: payData };
  const order = createOrderForTask(task, proofWithQr);
  db.orders[order.id] = order;

  // Persist to SQLite and run fulfillment state machine: planned→confirmed→executing→delivered
  try {
    upsertOrderSqlite(order);
    fulfillment.confirmOrder(order.id, { source: payData.source || "mcp", merchantRef });
    fulfillment.executeOrder(order.id);
    const deliverResult = fulfillment.deliverOrder(order.id);
    // Merge the certificateNo / qrCodeData from fulfillment into the in-memory order
    if (deliverResult && deliverResult.proof) {
      order.proof = { ...order.proof, ...deliverResult.proof };
      order.status = "delivered";
      order.lifecycle = (order.lifecycle || []).concat([
        { state: "confirmed",   label: "Confirmed",  at: order.createdAt, note: `Booking ref: ${merchantRef || "generated"}` },
        { state: "executing",   label: "In progress", at: order.createdAt, note: "Service delivery started." },
        { state: "delivered",   label: "Delivered",  at: deliverResult.deliveredAt, note: `Certificate: ${deliverResult.proof.certificateNo}` },
      ]);
      order.updatedAt = deliverResult.deliveredAt || nowIso();
      db.orders[order.id] = order;
    }
  } catch (fulfillErr) {
 ...
 *  Key-safe: requires "fieldName": { pattern to avoid matching field names
 *  that appear inside string values in the response. */
function extractJsonObjectField(text, fieldName) {
  // Match `"fieldName"` followed only by optional whitespace and a colon
  // This prevents matching the field name when it appears inside a string value.
...
// ── Receipt helpers ───────────────────────────────────────────────────────────
// receipts persisted to SQLite via upsertReceipt / getReceipt
 ...
 * Returns parsed JSON body on success, or null if webhook is not configured,
 * unreachable, non-2xx, or times out.
 */
async function _partnerWebhookPost(subpath, body, timeoutMs = 3000) {
  const webhookUrl = String(process.env.PARTNER_WEBHOOK_URL || "").trim();
  if (!webhookUrl) return null;
  try {
    const ac = new AbortController();
    const tid = setTimeout(() => ac.abort(), timeoutMs);
    const r = await fetch(`${webhookUrl}/${subpath}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ── /api/map-key — returns Amap JS SDK web key (browser-only, origin-gated) ──
    ...
    // ── /api/favorites — Per-device saved places (hotels/restaurants/attractions) ──
    ...
    const _favDeleteMatch = pathname.match(/^\/api\/favorites\/([^/]+)$/);
    if (req.method === "DELETE" && _favDeleteMatch) {
    ...
    if (req.method === "GET" && pathname === "/api/system/llm-status") {
      ...
      return writeJson(res, 200, {
        ...
        fx: {
          keyConfigured: Boolean(process.env.JUHE_KEY),
          source: process.env.JUHE_KEY ? "live" : "mock",
        },
        providers: {
          claude: {
    ...
    // POST /api/privacy/consent — Art. 7: Record consent grant or withdrawal
    if (req.method === "POST" && pathname === "/api/privacy/consent") {
      const body = await readBody(req);
      const errs = validateSchema(body, {
        granted:        { required: true, type: "boolean" },
        consentVersion: { required: true, type: "string", maxLength: 10 },
      ...
      if (!did) return writeJson(res, 400, { error: "missing_device_id", message: "X-Device-Id header required" });
      const result = gdpr.recordConsent({ deviceId: did, granted: body.granted, purposes: body.purposes, consentVersion: body.consentVersion, req });
      return writeJson(res, 200, { ok: true, ...result });
    }

    // GET /api/privacy/consent — Art. 7: Get consent status
    if (req.method === "GET" && pathname === "/api/privacy/consent") {
      const did = extractDeviceId(req, {});
      if (!did) return writeJson(res, 400, { error: "missing_device_id" });
      return writeJson(res, 200, { ok: true, ...gdpr.getConsentStatus(did) });
    }
    ...
    if (req.method === "POST" && pathname === "/api/privacy/erase") {
      const body = await readBody(req);
      const errs = validateSchema(body, { reason: { type: "string", maxLength: 500 } });
      if (errs.length) return writeJson(res, 400, { error: "validation_error", details: errs });
      const did = extractDeviceId(req, body);
      if (!did) return writeJson(res, 400, { error: "missing_device_id" });
      const result = gdpr.requestErasure(did, body.reason || "User requested erasure");
      return writeJson(res, 200, result);
    }

    // POST /api/privacy/restrict — Art. 18: Restrict processing
    if (req.method === "POST" && pathname === "/api/privacy/restrict") {
      const body = await readBody(req);
      const did = extractDeviceId(req, body);
      if (!did) return writeJson(res, 400, { error: "missing_device_id" });
      const result = gdpr.restrictProcessing(did, body.reasons || []);
      return writeJson(res, 200, result);
    }

    // DELETE /api/privacy/restrict — Art. 18: Withdraw restriction
    if (req.method === "DELETE" && pathname === "/api/privacy/restrict") {
      const did = extractDeviceId(req, {});
      if (!did) return writeJson(res, 400, { error: "missing_device_id" });
      return writeJson(res, 200, gdpr.withdrawRestriction(did));
    }
    ...
    if (req.method === "GET" && pathname === "/api/privacy/requests") {
      ...
      return writeJson(res, 200, { ok: true, requests: reqs });
    }

    // GET /api/privacy/notice — Art. 13/14: Machine-readable privacy notice
    if (req.method === "GET" && pathname === "/api/privacy/notice") {
      return writeJson(res, 200, gdpr.getPrivacyNotice());
    }
    ...
    if (req.method === "POST" && pathname === "/api/billing/settlements/run") {
      ...
      const rl = checkSettlementRateLimit(req);
      if (!rl.allowed) {
        res.setHeader("Retry-After", String(rl.retryAfterSec));
        return writeJson(res, 429, { error: "rate_limit_exceeded", message: `Settlement batch can run once per hour. Retry in ${rl.retryAfterSec}s.` });
      }
    ...
    if (req.method === "GET" && pathname === "/api/order/status") {
      ...
      if (ord.status === "pending") {
        ...
        if (age >= 2500) {
          ...
          saveDb();
          // Auto-signal "booking" to training example — strongest positive signal
          const _ordPlanId = ord.planId;
          if (_ordPlanId) setImmediate(() => {
            try {
              const { getTrainingExampleBySession: _getEx2 } = require("./src/services/db");
              const { recordSignal: _rSig2 } = require("./src/training/collector");
              const _ex2 = _getEx2(_ordPlanId);
              if (_ex2) _rSig2(_ex2.id, "booking");
            } catch {}
          });
        }
      }
...
${JSON.stringify(prevItinerary.card_data, null, 2).slice(0, 1200)}`
    ...
    // ── PATCH /api/plan/refine — Delta-patch refinement (no full replan) ──────
    if (req.method === "PATCH" && pathname === "/api/plan/refine") {
      const body = await readBody(req);
      const { sessionId, patch, message, currentCardData } = body;
      if (!sessionId) return writeJson(res, 400, { error: "missing sessionId" });
      try {
        const { getSession, patchSession: _patchSess } = require("./src/session/store");
        const { applyPlanPatch } = require("./src/session/updater");
        const session = getSession(sessionId);
        if (!session) return writeJson(res, 404, { error: "session_not_found" });

        // Build the refinement message: text-based (from chips/input) or structured patch (from sensitivity bar)
        let patchMessage = message;
        if (!patchMessage && patch && typeof patch === "object") {
          const patchDesc = Object.entries(patch)
            .filter(([, v]) => v != null)
            .map(([k, v]) => `${k}: ${v}`)
            .join(", ");
          if (!patchDesc) return writeJson(res, 400, { error: "empty_patch" });
          patchMessage = `\u6309\u4ee5\u4e0b\u8981\u6c42\u8c03\u6574\u65b9\u6848\uff1a${patchDesc}`;
          // "按以下要求调整方案：{patchDesc}"
        }
        if (!patchMessage) return writeJson(res, 400, { error: "missing message or patch" });

        const existingPlan = currentCardData || session.plan;
        if (!existingPlan) return writeJson(res, 404, { error: "no_plan_in_session" });
        ...
        if (!patchResult.ok) {
          return writeJson(res, 200, { ok: false, patchApplied: false, spokenText: patchResult.spokenText });
        }

        // Persist updated plan back to session
        _patchSess(sessionId, { plan: patchResult.patched });

        // Auto-signal "refine" to training example — user requested modification
        setImmediate(() => {
          try {
            const { getTrainingExampleBySession: _getEx } = require("./src/services/db");
            const { recordSignal: _rSig } = require("./src/training/collector");
            const _ex = _getEx(sessionId);
            if (_ex) _rSig(_ex.id, "refine");
          } catch {}
        });

        return writeJson(res, 200, {
          ok: true, sessionId,
          changedCards: patchResult.patched,
          patchApplied: true,
    ...
    if (req.method === "POST" && pathname === "/api/plan/feedback") {
      const body = await readBody(req);
      const { planId, deviceId, rating, comment, destination, durationDays } = body;
      if (!planId || !deviceId || rating == null) {
        return writeJson(res, 400, { error: "missing planId, deviceId, or rating" });
      }
      if (rating < 1 || rating > 5) {
        return writeJson(res, 400, { error: "rating must be 1-5" });
      }
      const { recordFeedback } = require("./src/training/collector");
      const { getTrainingExampleBySession } = require("./src/services/db");
      // Link rating to training example via session_id (planId = sessionId from SSE)
      const example = getTrainingExampleBySession(planId);
      const exampleId = example ? example.id : null;
      const feedbackId = recordFeedback({ planId, deviceId, rating: Number(rating), comment, exampleId, destination, durationDays });
      return writeJson(res, 200, { ok: true, feedbackId, exampleId });
    }

    // GET /api/training/stats — training data overview (admin)
    ...
    if (req.method === "GET" && pathname === "/api/training/export") {
      if (!requireAdmin(req, res)) return;
      const { searchParams: sp } = new URL(req.url, "http://localhost");
      const minScore   = parseFloat(sp.get("minScore") || "0.6");
    ...
    if (req.method === "POST" && pathname === "/api/training/finetune") {
      if (!requireAdmin(req, res)) return;
      const body = await readBody(req);
      ...
      try {
        const { exportFineTuningJSONL } = require("./src/training/collector");
        const jsonl = exportFineTuningJSONL({ minScore, limit });
        const lines = jsonl.split("\n").filter(Boolean).length;
        if (lines < 10) {
          return writeJson(res, 400, { error: `Insufficient training data: ${lines} examples (min 10 required)` });
        }
        ...
        form.append("file", Buffer.from(jsonl, "utf8"), { filename: "training.jsonl", contentType: "application/x-ndjson" });
    ...
    // GET /api/training/jobs — list fine-tuning jobs (admin)
    ...
    // GET /api/training/benchmark — run capability benchmark (admin, slow ~30s)
    if (req.method === "GET" && pathname === "/api/training/benchmark") {
      ...
      try {
        ...
        const { runBenchmark } = require("./src/training/benchmark");
    ...
    // GET /api/training/benchmark/history — past benchmark run summaries (admin)
    if (req.method === "GET" && pathname === "/api/training/benchmark/history") {
      if (!requireAdmin(req, res)) return;
      const { getBenchmarkHistory } = require("./src/training/benchmark");
      return writeJson(res, 200, { ok: true, runs: getBenchmarkHistory(20) });
    }

    // GET /api/training/prompt-stats — A/B prompt experiment results (admin)
    if (req.method === "GET" && pathname === "/api/training/prompt-stats") {
      if (!requireAdmin(req, res)) return;
      const { getPromptStats } = require("./src/ai/promptOptimizer");
      return writeJson(res, 200, { ok: true, experiments: getPromptStats() });
    }
...
7. Always respond in the same language the user uses (ZH/EN/JA/KO)`;
    ...
    if (req.method === "POST" && pathname === "/api/alipay/notify") {
      ...
      if (trade_status === "TRADE_SUCCESS" || trade_status === "TRADE_FINISHED") {
        const order = Object.values(db.orders).find(
          (o) => o.proof && o.proof.alipayTradeNo === out_trade_no
        );
        if (order) {
          order.paymentStatus = "paid";
          order.paidAt        = nowIso();
          lifecyclePush(order.lifecycle, "paid", "Payment confirmed", `Alipay ${out_trade_no}`);
          saveDb();
        }
      }
    ...
    // ── POST /api/agent/check — availability check (webhook or deterministic fallback) ──
    if (req.method === "POST" && pathname === "/api/agent/check") {
      const body = await readBody(req);
      const { candidateId = "", party_size = 2, time_constraint = "", budget = "mid" } = body;
      const webhookData = await _partnerWebhookPost("v1/availability", { candidateId, party_size, time_constraint, budget }, 2000);
      if (webhookData) return writeJson(res, 200, { ok: true, source: "webhook", ...webhookData });
      // Deterministic hash fallback — no I/O, pure function of inputs
      const h = Math.abs(Array.from(String(candidateId)).reduce((a, c) => (a * 31 + c.charCodeAt(0)) | 0, 7));
      const available = (h % 10) < 8; // ~80% available
      const queueEstimateMin = available ? (h % 6) * 5 : 0;
      const budgetMid = budget === "high" ? 350 : budget === "low" ? 80 : 160;
      const price = budgetMid + (h % 40) - 20;
      return writeJson(res, 200, { ok: true, source: "deterministic", available, queueEstimateMin, price });
    }

    // ── POST /api/agent/proof — generate HTML receipt ─────────────────────
    if (req.method === "POST" && pathname === "/api/agent/proof") {
      const body = await readBody(req);
    ...
    const pdfReceiptMatch = pathname.match(/^\/api\/orders\/([^/]+)\/receipt\/pdf$/);
    ...
    if (req.method === "POST" && pathname === "/api/agent/reserve") {
      ...
      const webhookData = await _partnerWebhookPost("v1/reserve", { candidateId, place, amount: Number(amount), party_size, time_constraint, railId }, 3000);
      if (webhookData) return writeJson(res, 200, { ok: true, source: "webhook", ...webhookData });
    ...
    // ── Order Lifecycle (Fulfillment State Machine) ───────────────────────────
    const orderLifecycleMatch = pathname.match(/^\/api\/orders\/([^/]+)\/(confirm|cancel|refund|proof)$/);
    if (orderLifecycleMatch) {
