...
// ── Receipt helpers ───────────────────────────────────────────────────────────
const receiptStore = new Map(); // orderId → html string
...
7. Always respond in the same language the user uses (ZH/EN/JA/KO)`;
    ...
    if (req.method === "POST" && pathname === "/api/agent/check") {
      ...
      if (webhookUrl) {
        try {
          ...
          const r = await fetch(`${webhookUrl}/v1/availability`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ candidateId, party_size, time_constraint, budget }),
            signal: ac.signal,
          });
          clearTimeout(tid);
          if (r.ok) {
    ...
    if (req.method === "POST" && pathname === "/api/agent/proof") {
      ...
      receiptStore.set(String(orderId), html);
    ...
    if (req.method === "GET" && /^\/api\/orders\/[^/]+\/receipt$/.test(pathname)) {
      const orderId = pathname.split("/")[3];
      const html = receiptStore.get(String(orderId));
      if (!html) return writeJson(res, 404, { error: "receipt_not_found" });
