...
function createPaymentRailManager(options = {}) {
  ...
  return {
    ...
    async charge({ railId, amount, currency = "CNY", userId = "demo", taskId = "" }) {
      ...
      // ── Live: Stripe PaymentIntent (card_delegate only) ──────────────────
      if (rail.id === "card_delegate") {
        const stripeKey = String(process.env.STRIPE_SECRET_KEY || "").trim();
        if (stripeKey) {
          try {
            const amountUsd = Math.round((Number(amount || 0) / 7.18) * 100); // USD cents
            const resp = await fetch("https://api.stripe.com/v1/payment_intents", {
              ...
              body: new URLSearchParams({
                ...
                currency: "usd",
                description: `CrossX ${taskId || refSeed}`,
                "metadata[taskId]": String(taskId || ""),
                "metadata[userId]": String(userId || ""),
                "metadata[cnyAmount]": String(Number(amount || 0).toFixed(2)),
              }),
              signal: AbortSignal.timeout(8000),
            });
            ...
            if (pi.id && pi.client_secret) {
              return {
                ok: true, latency, provider: "Stripe (live)", source: "stripe_payment_intent",
                sourceTs: new Date().toISOString(),
                data: {
                  amount: Number(amount || 0), currency,
                  paymentRef: `PAY-${refSeed}`, gatewayRef: pi.id,
                  railId: rail.id, railLabel: rail.label, userId, taskId, fx,
                  cardChargeId: pi.id,
            ...
            console.warn("[card_delegate] Stripe PI non-success:", pi.error);
          } catch (err) {
            console.warn("[card_delegate] Stripe fallback:", err.message);
          }
        }
        // Sandbox: no QR, return charge confirmation token
        const cardChargeId = `CARD-${refSeed}`;
        return {
          ok: true, latency, provider: rail.provider, source: "card_delegate_sandbox",
          sourceTs: new Date().toISOString(),
          data: {
            amount: Number(amount || 0), currency,
            paymentRef: `PAY-${refSeed}`, gatewayRef: `GW-${rail.id}-${refSeed}`,
            railId: rail.id, railLabel: rail.label, userId, taskId, fx,
            cardChargeId, sandbox: true,
          },
        };
      }

      // ── Sandbox fallback: QR deeplink for Alipay / WeChat ────────────────
