...
async function saveStore() {
  ...
  _writeChain = _writeChain
    .then(() => fs.promises.writeFile(STORE_PATH, JSON.stringify(_store), "utf8"))
    .catch((e) => console.warn("[rag/engine] saveStore error:", e.message));
  return _writeChain;
}
...
async function generateEmbedding(text, apiKey) {
  ...
  throw new Error(data.error?.message || "No embedding returned");
}
