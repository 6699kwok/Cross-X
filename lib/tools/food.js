...
function createFoodTools({ connectors, payments } = {}) {
  ...
  return {
    ...
    async makeProof(input) {
      ...
      // Use POI address from queryMap result if available; build Amap deep-link
      const placeName = String((input && input.place) || "餐厅");
      const cityName  = String((input && input.city)  || "上海");
      const amapQuery = encodeURIComponent(`${placeName} ${cityName}`);
      const navLink   = `https://uri.amap.com/search?keyword=${amapQuery}&sourceApplication=crossx`;
      const address    = (input && input.address) || `${cityName} · ${placeName}`;
      // Real booking/payment refs from prior execution steps (injected by runner)
      const bookingRef = (input && input.bookingRef) || (input && input.lockId) || null;
      const paymentRef = (input && input.paymentRef) || null;
      const itinerary  = bookingRef
        ? (liveTranslationOn
            ? `CN: 座位已确认 [${bookingRef}]${paymentRef ? `，支付: ${paymentRef}` : ""} / EN: Seat confirmed [${bookingRef}]${paymentRef ? `, payment: ${paymentRef}` : ""}`
            : `Seat confirmed [${bookingRef}]${paymentRef ? `, payment ref ${paymentRef}` : ""}`)
        : (liveTranslationOn
      ...
      return {
        ...
        mcpOp: "deliverable",
        data: {
          bilingualAddress: `CN: ${address} / EN: ${placeName}, ${cityName}`,
          navLink,
          itinerary,
          bookingRef,
          paymentRef,
