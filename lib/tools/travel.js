...
function createTravelTools({ connectors, payments } = {}) {
  ...
  return {
    ...
    async makeProof(input) {
      ...
      // Build Amap deep-link from destination coordinates or name
      const destName  = String((input && input.destination) || (input && input.city) || "目的地");
      const destCoord = (input && input.destinationCoord) ? String(input.destinationCoord) : null;
      const amapNavUrl = destCoord
        ? `https://uri.amap.com/navigation?to=${encodeURIComponent(destCoord)}&toname=${encodeURIComponent(destName)}&mode=car&sourceApplication=crossx`
        : `https://uri.amap.com/search?keyword=${encodeURIComponent(destName)}&sourceApplication=crossx`;
      // Real booking/payment refs from prior execution steps (injected by runner)
      const ticketRef  = (input && input.ticketRef)  || (input && input.bookingRef) || null;
      const paymentRef = (input && input.paymentRef) || null;
      const itinerary  = ticketRef
        ? (liveTranslationOn
            ? `CN: 票务已确认 [${ticketRef}]${paymentRef ? `，支付: ${paymentRef}` : ""} / EN: Ticket confirmed [${ticketRef}]${paymentRef ? `, payment: ${paymentRef}` : ""}`
            : `Ticket confirmed [${ticketRef}]${paymentRef ? `, payment ref ${paymentRef}` : ""}`)
        : (liveTranslationOn
      ...
      return {
        ...
        mcpOp: "deliverable",
        data: {
          bilingualAddress: `CN: ${destName} / EN: ${destName}`,
          navLink: amapNavUrl,
          itinerary,
          ticketRef,
          paymentRef,
