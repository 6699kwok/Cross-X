...
async function executePlan({ plan, tools, amount, currency, userId, taskId, paymentRail }) {
  ...
  for (const step of plan.steps) {
    ...
    // Gather key results from prior completed steps for context injection
    const priorBook    = outputs.find((o) => o.toolType === "book.lock" || o.toolType === "transport.lock");
    const priorPay     = outputs.find((o) => o.toolType === "pay.act");
    const priorMap     = outputs.find((o) => o.toolType === "map.query" || o.toolType === "route.plan");
    const bookData     = (priorBook && priorBook.data) || {};
    const payData      = (priorPay  && priorPay.data)  || {};
    const mapData      = (priorMap  && priorMap.data)   || {};
    ...
    const input =
      step.toolType === "pay.act"
        ...
        ? {
            ...
            // Real data from prior steps
            place:        mapData.picks?.[0]?.name || plan?.constraints?.destination || plan?.constraints?.city || "",
            address:      mapData.picks?.[0]?.address || "",
            destinationCoord: plan?.constraints?.destinationCoord || null,
            bookingRef:   bookData.lockId || bookData.ticketRef || null,
            lockId:       bookData.lockId || null,
            ticketRef:    bookData.ticketRef || null,
            paymentRef:   payData.paymentRef || null,
            paymentRail:  payData.railId || payData.railLabel || null,
            amount,
            currency,
          }
        : {
